...
class TimeSeriesEncoderClassifier(sklearn.base.BaseEstimator,
    ...
    def __init__(self, compared_length, nb_random_samples, negative_penalty,
        ...
        self.loss = losses.triplet_loss_ts_rep.TripletLoss(
            compared_length, nb_random_samples, negative_penalty, device, latent_shape
        )
        self.loss_varying = losses.triplet_loss_ts_rep.TripletLossVaryingLength(
            compared_length, nb_random_samples, negative_penalty, device, latent_shape
        )
    ...
    def fit_encoder(self, X, y=None, save_memory=False, verbose=False, drop_last=False):
        """
        ...
        train_generator = torch.utils.data.DataLoader(
            train_torch_dataset, batch_size=self.batch_size, shuffle=True, drop_last=drop_last
        )
        ...
        while i < self.nb_steps:
            train_loss = 0 # track train loss per epoch here
            if verbose:
                print('Epoch: ', epochs + 1)
            for batch_idx, batch in train_generator:
                if self.cuda:
                    batch = batch.cuda(self.gpu)
                self.optimizer.zero_grad()
                if not varying:
                    loss = self.loss(batch_idx, batch, 
                        self.encoder, train, epochs+1, save_memory=save_memory
                    )
                else:
                    loss = self.loss_varying(batch_idx, batch,
                        self.encoder, train, epochs+1, save_memory=save_memory
                    )
                loss.backward()
                self.optimizer.step()
                train_loss += loss.item() # add bacth loss
                i += 1
                if i >= self.nb_steps:
                    break
            train_loss_epoch = train_loss/ len(train_generator)
    ...
    def encode(self, X, batch_size=50):
        ...
        with torch.no_grad():
            if not varying:
                for batch_idx, batch in test_generator:
                    if self.cuda:
                        batch = batch.cuda(self.gpu)
                    features[
...
class CausalCNNEncoderClassifier(TimeSeriesEncoderClassifier):
    ...
    def __init__(self, compared_length=50, nb_random_samples=10,
                 ...
                 in_channels=1, cuda=False, gpu=0, latent_shape=(800,100)):
        super(CausalCNNEncoderClassifier, self).__init__(
            ...
            in_channels, out_channels, cuda, gpu, latent_shape
        )
    ...
    def set_params(self, compared_length, nb_random_samples, negative_penalty,
                   ...
                   in_channels, cuda, gpu, latent_shape):
        self.__init__(
            compared_length, nb_random_samples, negative_penalty, batch_size,
